...
def combine_and_mask_train(img_new, mask, img_black, index):
    """
    Combine img_new, mask, and image_black based on the mask
    ...
    h = img_black.size[0]
    w = img_black.size[1]
    black_arr = np.zeros((w,h,3), dtype = "uint8")
    if np.random.uniform() < 0.8:
        for i in range(3):
            black_arr[:,:,i]=colours[index][i]
    else:
        rand_idx = np.random.randint(0,200)
        for i in range(3):
            black_arr[:,:,i]=colours[rand_idx][i]
    
    img_resized = crop_and_resize(Image.fromarray(black_arr), img_black)
    img_resized_np = np.asarray(img_resized)
    # Mask new img
    img_masked_np = np.around(img_resized_np * (1 - mask)).astype(np.uint8)
...
def combine_and_mask_test(img_new, mask, img_black, index):
    """
    Combine img_new, mask, and image_black based on the mask
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ood_dirname', default='CUB_ood', help='Name of fixed dataset: class <-> place association fixed at train, swapped at test')
    ...
    for idx,spc in enumerate(species,1):
        ...
        ood_spc_train_dir = os.path.join(args.out_dir, args.ood_dirname, 'train', spc)
        ood_spc_test_in_dir = os.path.join(args.out_dir, args.ood_dirname, 'test_in', spc)
        ood_spc_test_out_dir = os.path.join(args.out_dir, args.ood_dirname, 'test_out', spc)
        ...
        os.makedirs(ood_spc_train_dir, exist_ok=True)
        os.makedirs(ood_spc_test_in_dir, exist_ok=True)
        os.makedirs(ood_spc_test_out_dir, exist_ok=True)
        ...
        for img_path, seg_path, train_place_path, test_place_path in it:
            ...
            img_train = combine_and_mask_train(train_place, seg_np, img_black, idx)
            img_test_out = combine_and_mask_test(test_place, seg_np, img_black, idx)
            img_test_in = combine_and_mask_train(test_place, seg_np, img_black, idx)

            full_train_path = os.path.join(ood_spc_train_dir, img_path)
            img_train.save(full_train_path)
            full_test_in_path = os.path.join(ood_spc_test_in_dir, img_path)
            img_test_in.save(full_test_in_path)
            full_test_out_path = os.path.join(ood_spc_test_out_dir, img_path)
            img_test_out.save(full_test_out_path)
